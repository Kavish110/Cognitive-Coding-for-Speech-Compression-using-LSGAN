...
def causal_conv1d(in_ch, out_ch, k, stride=1, dilation=1):
    # pad for causal conv with dilation
    pad = (k - 1) * dilation
...
class LinearGRUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.W_zr = nn.Linear(input_size, 2 * hidden_size)
        self.U_zr = nn.Linear(hidden_size, 2 * hidden_size, bias=False)
    ...
    def forward(self, x_t, h):
        ...
        # candidate uses linear activation (no tanh); apply r gating to hidden before U_n
        n = self.W_n(x_t) + self.U_n(r * h)
        h_new = (1 - z) * h + z * n
        return h_new
...
class CCEncoder(nn.Module):
    def __init__(self, sr=16000,
        ...
        super().__init__()
        self.sr = sr
        # provide sensible default configs if None to avoid TypeError when indexing
